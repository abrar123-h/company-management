...
try:
    ...
    cursor.execute(
        ...
        (company_name, company_description, creation_date)
...
try:
    ...
    serial_code = "PROD-" + uuid.uuid4().hex[:8].upper()
...
try:
    ...
    for row in rows:
        result.append({
            ...
            "company_id": row[1],
            "product_name": row[2],
            "quantity": str(row[3]) if row[3] is not None else "0",
            "number_of_items": (
...
try:
    ...
    try:
        ...
        return jsonify({
            "message":
                "Product stock must be numeric for stock assignment"
        }), 400
    ...
    remaining_quantity = (
        available_quantity - assign_quantity
    )

    total_amount = (
        assign_quantity * retail_price
    )
...
try:
    ...
    for row in rows:
        result.append({
            ...
            "remaining_stock": (
                int(row[8]) if str(row[8]).isdigit() else row[8]
            ),
